function matt = InfoRatio(x,xret,aret,t,bidask_spread,vol_target,vol,forecastscalar)
%EWMAC calculates the sharpe ratio of trading rule
%Input: x: time series of instrument
%       xret: returns of instrument
%       aret: active return vs its benchmark
...
% nants=x(isnan(x));
% x_exnan=x(~isnan(x));
rollSR=smartMovingAvg(aret,t)./smartMovingStd(aret,t)*sqrt(250);

%raw_ewma_crossover=ewma_fast-ewma_slow; %macd crossing


if strcmp(vol,'')
    vol=smartMovingStd(aret,25);
end
